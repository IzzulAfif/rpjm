...
|---------------------------------
*/
class C_usulanduniausaha extends IAN_Controller{

	public function __construct(){
		parent::__construct();
		$this->load->model('usulanduniausaha/m_usulanduniausaha');
	}

	public function getUsulanDuniausaha(){
        $start      = ($this->input->post('start', TRUE) ? $this->input->post('start', TRUE) : 0);
        $limit      = ($this->input->post('limit', TRUE) ? $this->input->post('limit', TRUE) : 20);
		
		$result 		= $this->m_usulanduniausaha->getGridUsulanDuniausaha($start, $limit);
		$resultCount 	= $this->m_usulanduniausaha->countGridUsulanDuniausaha();
	...
	public function delUsulanDuniausaha(){
		$kode_musrenbang = ($this->input->post('kode_musrenbang', TRUE) ? $this->input->post('kode_musrenbang', TRUE) : '');
		$cekResult = $this->m_usulanduniausaha->cekRelasi($kode_musrenbang);

		if($cekResult == 0){
			$this->m_usulanduniausaha->deleteUsulanDuniausaha($kode_musrenbang);
			$data['msg']=0;
		} else {
			$data['msg']=1;
		}
	...
	public function saveUsulanDuniausaha(){
		$table 				= "trs_musrenbang";
		$kode_musrenbang 	= $this->m_usulanduniausaha->getUnikKode($table);
		$tahun				= date('Y')+1;
		$kode_jenisusulan	= 'JU007';
    	...
    	if($kode_musrenbang==""){
    		$success = 3;
    	} elseif($this->m_usulanduniausaha->cekData($kode_musrenbang) == 0){
    		$this->m_usulanduniausaha->saveUsulanDuniausaha($kode_musrenbang, $tahun, $kode_jenisusulan, $created, $createdby, $kegiatan, $lokasi, $volume, $kode_satuan, $swadana, $apbd_kab, $apbd_prov, $apbn, $latitude, $longitude);
    		$success = 1;
    	} else {
    		$success = 2;
    	}
	...
	public function editUsulanDuniausaha(){
    	...
    	if(empty($kode_musrenbang)){
    		...
    		$this->m_usulanduniausaha->updateUsulanDuniausaha($kode_musrenbang, $updated, $updateby, $kegiatan, $lokasi, $volume, $kode_satuan, $swadana, $apbd_kab, $apbd_prov, $apbn, $latitude, $longitude);
    		$success = 1;
    	}
	...
	public function searchUsulanDuniausaha(){
		$name 		= ($this->input->post('name', TRUE) ? $this->input->post('name', TRUE) : '');
    	$result 	= $this->m_usulanduniausaha->searchUsulanDuniausaha($name);
    	foreach ($result->result() as $key => $value) {
			$data['data'][]=array(
				'id_musrenbang'			=> $value->id_musrenbang,
				'kode_musrenbang'	=> $value->kode_musrenbang,
