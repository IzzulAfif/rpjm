...
|---------------------------------
*/
class C_usulanakademisi extends IAN_Controller{

	public function __construct(){
		parent::__construct();
		$this->load->model('usulanakademisi/m_usulanakademisi');
	}

	public function getUsulanAkademisi(){
        $start      = ($this->input->post('start', TRUE) ? $this->input->post('start', TRUE) : 0);
        $limit      = ($this->input->post('limit', TRUE) ? $this->input->post('limit', TRUE) : 20);
		
		$result 		= $this->m_usulanakademisi->getGridUsulanAkademisi($start, $limit);
		$resultCount 	= $this->m_usulanakademisi->countGridUsulanAkademisi();
	...
	public function delUsulanAkademisi(){
		$kode_musrenbang = ($this->input->post('kode_musrenbang', TRUE) ? $this->input->post('kode_musrenbang', TRUE) : '');
		$cekResult = $this->m_usulanakademisi->cekRelasi($kode_musrenbang);

		if($cekResult == 0){
			$this->m_usulanakademisi->deleteUsulanAkademisi($kode_musrenbang);
			$data['msg']=0;
		} else {
			$data['msg']=1;
		}
	...
	public function saveUsulanAkademisi(){
		$table 				= "trs_musrenbang";
		$kode_musrenbang 	= $this->m_usulanakademisi->getUnikKode($table);
		$tahun				= date('Y')+1;
		$kode_jenisusulan	= 'JU005';
    	...
    	if($kode_musrenbang==""){
    		$success = 3;
    	} elseif($this->m_usulanakademisi->cekData($kode_musrenbang) == 0){
    		$this->m_usulanakademisi->saveUsulanAkademisi($kode_musrenbang, $tahun, $kode_jenisusulan, $created, $createdby, $kegiatan, $lokasi, $volume, $kode_satuan, $swadana, $apbd_kab, $apbd_prov, $apbn, $latitude, $longitude);
    		$success = 1;
    	} else {
    		$success = 2;
    	}
	...
	public function editUsulanAkademisi(){
    	...
    	if(empty($kode_musrenbang)){
    		...
    		$this->m_usulanakademisi->updateUsulanAkademisi($kode_musrenbang, $updated, $updateby, $kegiatan, $lokasi, $volume, $kode_satuan, $swadana, $apbd_kab, $apbd_prov, $apbn, $latitude, $longitude);
    		$success = 1;
    	}
	...
	public function searchUsulanAkademisi(){
		$name 		= ($this->input->post('name', TRUE) ? $this->input->post('name', TRUE) : '');
    	$result 	= $this->m_usulanakademisi->searchUsulanAkademisi($name);
    	foreach ($result->result() as $key => $value) {
			$data['data'][]=array(
				'id_musrenbang'			=> $value->id_musrenbang,
				'kode_musrenbang'	=> $value->kode_musrenbang,
