...
|---------------------------------
*/
class C_usulankomunitas extends IAN_Controller{

	public function __construct(){
		parent::__construct();
		$this->load->model('usulankomunitas/m_usulankomunitas');
	}

	public function getUsulanKomunitas(){
        $start      = ($this->input->post('start', TRUE) ? $this->input->post('start', TRUE) : 0);
        $limit      = ($this->input->post('limit', TRUE) ? $this->input->post('limit', TRUE) : 20);
		
		$result 		= $this->m_usulankomunitas->getGridUsulanKomunitas($start, $limit);
		$resultCount 	= $this->m_usulankomunitas->countGridUsulanKomunitas();
	...
	public function delUsulanKomunitas(){
		$kode_musrenbang = ($this->input->post('kode_musrenbang', TRUE) ? $this->input->post('kode_musrenbang', TRUE) : '');
		$cekResult = $this->m_usulankomunitas->cekRelasi($kode_musrenbang);

		if($cekResult == 0){
			$this->m_usulankomunitas->deleteUsulanKomunitas($kode_musrenbang);
			$data['msg']=0;
		} else {
			$data['msg']=1;
		}
	...
	public function saveUsulanKomunitas(){
		$table 				= "trs_musrenbang";
		$kode_musrenbang 	= $this->m_usulankomunitas->getUnikKode($table);
		$tahun				= date('Y')+1;
		$kode_jenisusulan	= 'JU006';
    	...
    	if($kode_musrenbang==""){
    		$success = 3;
    	} elseif($this->m_usulankomunitas->cekData($kode_musrenbang) == 0){
    		$this->m_usulankomunitas->saveUsulanKomunitas($kode_musrenbang, $tahun, $kode_jenisusulan, $created, $createdby, $kegiatan, $lokasi, $volume, $kode_satuan, $swadana, $apbd_kab, $apbd_prov, $apbn, $latitude, $longitude);
    		$success = 1;
    	} else {
    		$success = 2;
    	}
	...
	public function editUsulanKomunitas(){
    	...
    	if(empty($kode_musrenbang)){
    		...
    		$this->m_usulankomunitas->updateUsulanKomunitas($kode_musrenbang, $updated, $updateby, $kegiatan, $lokasi, $volume, $kode_satuan, $swadana, $apbd_kab, $apbd_prov, $apbn, $latitude, $longitude);
    		$success = 1;
    	}
	...
	public function searchUsulanKomunitas(){
		$name 		= ($this->input->post('name', TRUE) ? $this->input->post('name', TRUE) : '');
    	$result 	= $this->m_usulankomunitas->searchUsulanKomunitas($name);
    	foreach ($result->result() as $key => $value) {
			$data['data'][]=array(
				'id_musrenbang'			=> $value->id_musrenbang,
				'kode_musrenbang'	=> $value->kode_musrenbang,
