...
class M_menu extends CI_Model
{
    ...
    public function getGridMenu($limit, $offset)
    {
        ...
        $db->select("id_seq AS id_seq, CAST(id_menu as varchar(10)) AS id, name AS name, parent AS parent, icon AS icon, CASE WHEN isactive = 'Y' THEN 1 ELSE 0 END AS isactive", FALSE);
        $db->from('sys_menu');
    ...
    public function saveMenu($id, $name, $parent, $icon, $isactive)
    {
        ...
        $db   = $this->getConnection();
            ...
            if($parent > 0){
                ...
                $cls = 'RPJM.module.'.$nameParent.'.'.$name.'.view.'.$name;
                $db->set('leaf', 'TRUE');
    ...
    public function updateMenu($id_seq, $id, $name, $parent, $icon, $isactive)
    {
        $this->setConnection('dbsystem');
        $db   = $this->getConnection();
        ...
        if($parent > 0){
            $nameParent = $this->getParent($parent);
            $cls = 'RPJM.module.'.$nameParent.'.view.'.$name;
            $data = array(
                           'id_menu'        => $id,
                           'name'           => $name,
                           'parent'         => $parent,
