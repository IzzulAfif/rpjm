...
class c_menu extends IAN_Controller
{
  ...
  {
    $id_seq       = ($this->input->post('id_seq', TRUE) ? $this->input->post('id_seq', TRUE) : '');
    $id           = ($this->input->post('id', TRUE) ? $this->input->post('id', TRUE) : '');
    $name         = ($this->input->post('name', TRUE) ? $this->input->post('name', TRUE) : '');
    $parent1      = ($this->input->post('parent', TRUE) ? $this->input->post('parent', TRUE) : '');
    if($parent1 == NULL || $parent1 == '') { $parent = 0; } else { $parent = $parent1; }  
    ...
    if($name == '' || $name == NULL){ 
      $success = 0;
    } else if($this->m_menu->cekMenuID($name, $id) == 1){ 
      $this->m_menu->updateMenu($id_seq, $id, $name, $parent, $icon, $isactive);
      $success = 1;
    } else { 
      $success = 2; 
    }
